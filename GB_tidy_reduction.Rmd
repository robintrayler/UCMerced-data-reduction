...
The $\delta^{13}C$ and $\delta^{18}O$ values of all samples were measured using a __Thermo-Finnigan Gasbench II__ coupled with a __Delta V Plus Continuous Flow Isotope Ratio Mass Spectrometer__ housed in the Stable Isotope Facility at the University of California, Merced.
...
file_path <- '~/Box Sync/Data Repository/Gasbench/GB20201211.csv'
```
...
standards_used = c('NBS 18', 'USGS 44', 'CM')
# material to use for linearity corrections
linearity_material = c('NBS 18', 'CM', 'USGS 44')
# material to use for drift corrections
drift_material = c('NBS 18')
...
A locally smoothed polynomial (loess) was fit to mass 44 peak area (mV/s) versus the pooled within sample $\delta^{18}O$ deviations for all analyses. Since a linearity correction was `r ifelse(oxygen_linearity,' ','not')` requested, A the trend was `r ifelse(oxygen_linearity,' ','not')` subtracted from the raw data.
...
if(carbon_drift) {
  ...
  data <- data %>% 
    mutate(d13C_measured = d13C_measured - predict(C_drift_fit, newdata = data)) %>%
    group_by(id2) %>%
    mutate(d13C_dev = d13C_measured - mean(d13C_measured)) %>%
    ungroup()
}
...
```{r, echo = FALSE, message = FALSE}
final_data %>% arrange(sample) %>% write_csv(path = paste0('./GB_results/', file_name,'_corrected.csv'))
raw_data %>% write_csv(path = paste0('./GB_results/', file_name, '_raw.csv'))
