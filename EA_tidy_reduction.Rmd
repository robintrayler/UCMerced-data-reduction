...
file_path <- '~/Box Sync/Data Repository/EA/EA20210107.csv'
```
...
drift_material = c('USGS 41a', 'costech acetanilide')
...
```{r, echo = FALSE, message = FALSE}
final_data %>% arrange(sample) %>% write_csv(path = paste0('./EA_results/', file_name,'_corrected.csv'))
raw_data %>% write_csv(path = paste0('./EA_results/', file_name, '_raw.csv'))
file.copy(from = 'EA_tidy_reduction.Rmd', to = paste0('./EA_results/', file_name,'_code.Rmd'))
```